...
public class PracticeProblem {
	...
	public static final int LARGE = 2^31;

	public static int goodString(String que) {
		boolean lengthGood = que.length() >= 2;
		boolean subpalindromeGood = que.charAt(0) == que.charAt(que.length()-1);

		if (lengthGood && subpalindromeGood) {
			return 0;
		}
		// length >=2 means that the only requirement is that there are two of the same letter

		System.out.println("Size:" + que.length());

		int leastSteps = LARGE;
		for (int x = 0; x < que.length(); x++) {
			for (int y = 0; y < que.length(); y++) {

				if (x == y) { continue; }

				if (que.charAt(x) == que.charAt(y)) {

					int xFar0 = x - 1;
					int xFarLen = que.length() - x;
					int yFar0 = y - 1;
					int yFarLen = que.length() - y;

					int xClosest = Math.min(xFar0, xFarLen);
					
					int yClosest = (xClosest == xFar0) ? yFarLen : yFar0;

					leastSteps = Math.min(leastSteps, xClosest + yClosest);
					System.out.println("found result:" + x + ", " + y + " with size " + (xClosest + yClosest));
				}
...
[![Open in Codespaces](https://classroom.github.com/assets/launch-codespace-2972f46106e565e64193e422d61a12cf1da4916b45550586e14ef0a7c637dd04.svg)](https://classroom.github.com/open-in-codespaces?assignment_repo_id=18461537)
# Instructions  
